...
def get_translation_model():
    """
    直接建立一個獨立的 Gemini model 專門做整句翻譯。
    優先沿用 services/ai_service.py 在 import 時已設定好的 genai.configure(api_key=...)；
    如果 Secrets 裡有讀得到 Gemini 金鑰，也明確再設定一次，避免兩邊沒有共用到金鑰設定。
    """
    api_key = (
    ...
    return None  # 實際 model 由 translate_full_sentence 動態嘗試多個候選名稱


TRANSLATION_MODEL_CANDIDATES = [
    "gemini-2.0-flash", "gemini-1.5-flash", "gemini-2.0-flash-exp",
    "gemini-1.5-pro", "gemini-pro",
]


def translate_full_sentence(text: str) -> dict | None:
    """把整句話丟給 Gemini，要求『翻譯整句』而不是『挑字解釋』。"""
    get_translation_model()  # 確保金鑰已設定
    prompt = f"""你是專業的法文/英文翻譯與語言教學助手。
請將下面這一整段話「完整翻譯」成繁體中文，不要只挑其中一個單字解釋，
也不要省略或改寫原文的任何部分。同時判斷這段話主要是法文還是英文。

只回傳一個合法的 JSON 物件，不要有其他文字、不要加上 ```json 或任何 Markdown 標記：

原文：{text}

JSON 格式（欄位名稱與大小寫必須完全一致）：
{{"original_sentence": "原文，完全保留不變", "lang_code": "fr 或 en", "sentence_translation": "整句流暢通順的繁體中文翻譯"}}
"""
    last_error = None
    for model_name in TRANSLATION_MODEL_CANDIDATES:
        try:
            ...
            st.session_state["_debug_translate_error"] = ""
            st.session_state["_debug_translate_model"] = model_name
            return result
        except Exception as exc:
            last_error = f"[{model_name}] {type(exc).__name__}: {exc}"
            continue
    print(f"整句翻譯失敗: {last_error}")
    st.session_state["_debug_translate_error"] = last_error or "未知錯誤"
    return None
...
def analyze_sentence(sentence_input: str) -> None:
    """整句翻譯：直接呼叫獨立的 translate_full_sentence()，
    確保是翻譯『整句』而不是只解釋句子裡的某一個單字。"""
    ...
    if not data:
        ...
        try:
            data = translate_full_sentence(normalized)
        except Exception:
            data = None
        finally:
            loading.empty()
        if not data:
            debug_msg = st.session_state.get("_debug_translate_error", "")
            st.error(f"整句翻譯失敗，AI 服務可能忙碌中，請稍候一分鐘再試。\n\n除錯訊息：{debug_msg}")
            return
        st.session_state[cache_key] = data
        save_word_to_supabase({
            "word":data.get("original_sentence", normalized), "lang_code":data.get("lang_code", "fr"),
            "phonetic":"", "meaning":data.get("sentence_translation", ""),
            "example_sentence":"", "status":"review",
        })
