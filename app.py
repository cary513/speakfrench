...
<style>
body{{margin:0;font-family,sans-serif;color:#171717}}
.scene{{position;height:175px;overflow;border-radius:24px;background:#fff}}
.paris{{position;left:24px;top:22px;color:#999;letter-spacing:.18em;font-size:11px}}
.tower{{position;right:38px;bottom:18px;font-size:72px;color:#3a3a3a;transform(.7)}}
.path{{position;left:30px;right:70px;bottom:32px;border-bottom:3px dotted #F7A916}}
.traveller{{position;left:28px;bottom:24px;color:#F7A916;font-size:20px;transition .4s linear}}
.clock{{text-align;font-size:42px;font-weight:750;padding-top:55px;letter-spacing:-.03em}}
.goal{{text-align;color:#7D818A;margin-top:7px;font-size:14px}}
</style>
<script>
...
horizontal=True, format_func=lambda v"{MOOD_ICONS[MOODS.index(v)]} {v}",
)
...
row = rows[0]
data = {
"word".get("word", normalized), "lang_code".get("lang_code", "fr"),
"phonetic".get("phonetic", ""), "meaning".get("meaning", ""),
"example_sentence".get("example_sentence", ""), "sentence_translation":"",
}
except Exception:
data = None
if not data:
...
"word".get("word", normalized), "lang_code".get("lang_code", "fr"),
"phonetic".get("phonetic", ""), "meaning".get("meaning", ""),
"example_sentence".get("example_sentence", ""), "status":"review",
})
...
if name and goal:
profile = {"display_name", "current_level", "learning_goal", "interests"}
...
if left.button("播放", key=f"phrase_audio_{category}{index}", use_container_width=True):
try:
...
st.warning("語音暫時無法播放。")
if right.button("加入複習卡", key=f"phrase_save{category}_{index}", use_container_width=True):
save_word_to_supabase({
"word", "lang_code":"fr", "phonetic",
"meaning", "example_sentence", "status":"review",
})
...
f"""
<style>
.cal{{display;grid-template-columns(7,1fr);gap:8px}}
.dow{{text-align;color:#92959C;font-size:.75rem;padding:8px 0}}
.day{{min-height:54px;display;flex-direction;align-items;justify-content;border-radius:14px;color:#171717}}
...
total = {
"study_seconds"(int(log.get("study_seconds", 0)) for log in logs),
"remembered_cards"(int(log.get("remembered_cards", 0)) for log in logs),
"quiz_count"(int(log.get("quiz_count", 0)) for log in logs),
}
...
skills = {"全部", "閱讀 Reading":"reading", "聽力 Listening":"listening", "寫作 Writing":"writing", "口說 Speaking":"speaking"}
...
content:""; position; bottom:1px; width:18px; height:3px;
border-radius:999px; background(--lg-orange);
}}
</style>
""",
